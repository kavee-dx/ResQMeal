...
export function validateDonationForm(
  ...
  if (!values.numberOfPortions.trim()) {
    ...
    if (isNaN(portions)) {
      errors.numberOfPortions = 'Number of portions must be a valid number';
    } else if (portions <= 0) {
      errors.numberOfPortions = 'Number of portions must be greater than 0';
    } else if (!Number.isInteger(portions)) {
      errors.numberOfPortions = 'Number of portions must be a whole number';
    }
  }
  ...
  if (!values.preparationTime.trim()) {
    ...
    if (isNaN(preparationDate.getTime())) {
      errors.preparationTime = 'Please enter a valid preparation date and time';
    }
  }
  ...
  if (!values.expiryTime.trim()) {
    ...
    if (isNaN(expiryDate.getTime())) {
      errors.expiryTime = 'Please enter a valid expiry date and time';
    }
  ...
  } else if (!validStorageConditions.includes(values.storageCondition)) {
    errors.storageCondition = 'Please select a valid storage condition';
  }
  ...
  if (!values.packagingCondition.trim()) {
    errors.packagingCondition = 'Packaging condition is required';
